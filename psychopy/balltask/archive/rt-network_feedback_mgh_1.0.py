...
if SHAM and expInfo['feedback_on'] == 'Feedback':
    ...
    # Ensure the folder existsfilename_prefix
...
if group == '1':
    #pda_val = roi_raw_activations[roi_names_list.index('cen')] - roi_raw_activations[roi_names_list.index('dmn')]
    pda_label = 'cen_dmn_pda'
    cumulative_hit_labels = ['cen_cumulative_hits', 'dmn_cumulative_hits']
elif group == '0':
    #pda_val = roi_raw_activations[roi_names_list.index('smcl')] - roi_raw_activations[roi_names_list.index('smcr')]
...
if group == '1':
    ...
    roi_names_list = ['smcl', 'smcr','cen', 'dmn']
    cumulative_hits_fields = ['smcl_cumulative_hits', 'smcr_cumulative_hits']
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    with open(csv_path, 'a') as csvfile:
        ...
        stim_writer.writerow([
            ...
            *roi_raw_activations,
            'baseline',
            0, 0,  # placeholder for cumulative hits
            np.nan,  # num_pda_outliers during baseline
            pda_val,
            ball.pos[1],
...
while not (SHAM and expInfo['feedback_on'] == 'Feedback') and continueRoutine and routineTimer.getTime() > 0:
    ...
    # a list of [CEN, DMN] for the current frame
		...
		triggerClock.getTime(),
		triggerClock.getTime() + 1.2,
		*roi_all_for_csv,  # [smcl, smcr, cen, dmn]
		'feedback',
		hit_counter[0],
		hit_counter[1],
		num_pda_outliers,
